                ...
                <article wire:key="history-{{ $attempt->id }}" @class([
                    ...
                    <div class="border-t border-slate-100 bg-slate-50/40 px-5 py-4">
                        ...
                        <a href="{{ route('peserta.exam.review', $attempt) }}"
                           wire:navigate
                           @if ($loop->first)
                               id="feature-review-button"
                           @endif
                           @class([
                               'ui-btn-primary inline-flex w-full items-center justify-center gap-2 sm:w-auto',
                               'ui-tour-pointer' => $loop->first && in_array($focusHighlight ?? null, ['review', 'psychology'], true),
                           ])>
