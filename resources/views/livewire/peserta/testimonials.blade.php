<div class="min-h-screen bg-gradient-to-b from-slate-50 to-violet-50/30">
    <main class="mx-auto max-w-screen-2xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
        ...
        <div class="mb-8 rounded-2xl bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 p-6 text-white shadow-xl shadow-violet-500/20 sm:p-8">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <p class="text-xs font-bold uppercase tracking-widest text-violet-200">Wall of Love</p>
                ...
                <div class="flex items-center gap-2 rounded-xl bg-white/15 px-3 py-2 text-sm font-semibold ring-1 ring-white/20">
                    <svg class="h-4 w-4 text-violet-200" fill="currentColor" viewBox="0 0 24 24"><path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                    +200 XP Reward
                </div>
            </div>
        </div>
        ...
        <div class="mb-6 grid gap-3 sm:grid-cols-2">
            ...
            <button type="button" wire:click="openForm" class="ui-card flex items-center gap-3 p-4 text-left transition hover:ring-2 hover:ring-violet-200">
            ...
            <div class="mb-8 ui-card overflow-hidden">
                ...
                <form wire:submit="submit" class="space-y-6 p-6">
                            ...
                            @foreach ($featureTagOptions as $tag)
                                <button
                                    ...
                                    @class([
                                        ...
                                        'bg-violet-600 text-white ring-violet-600' => in_array($tag->value, $selectedTags, true),
                                        'bg-white text-slate-600 ring-slate-200 hover:ring-violet-300' => ! in_array($tag->value, $selectedTags, true),
                                    ])
                                >
                                    {{ $tag->hashtag() }}
                                </button>
                    ...
                    <label class="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-3">
                        <input type="checkbox" wire:model="isAnonymous" class="rounded border-slate-300 text-violet-600 focus:ring-violet-500">
                        <div>
        ...
        @if ($testimonials->isEmpty())
            <div class="ui-card px-6 py-16 text-center">
                <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-violet-100 text-violet-600">
                ...
                <button type="button" wire:click="openForm" class="ui-btn-primary mt-6">Tulis Testimoni Pertama</button>
            </div>
        @else
            ...
            @if ($hasMorePages)
                ...
                >
                    <div wire:loading wire:target="loadMore" class="flex items-center gap-2 text-sm font-medium text-slate-500">
                        <svg class="h-5 w-5 animate-spin text-violet-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
